package com.tinytree.redis.book;
...
public class Chapter05 {
    ...
    public void logRecent(Jedis conn, String name, String message, String severity) {
        ...
        Pipeline pipe = conn.pipelined();//使用流水线来将通信往返次数降低为一次
        pipe.lpush(destination, TIMESTAMP.format(new Date()) + ' ' + message);//将消息添加到日志表里面
        pipe.ltrim(destination, 0, 99);//修剪日志,让它只包含最新的100条消息
        pipe.sync();
    }
